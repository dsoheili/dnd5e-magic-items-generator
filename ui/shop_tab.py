...
class ShopTab:
   ...
   def create_shop_buttons(self, parent):
       ...
       buttons_data = [
           ("✨ Magic Items Shop", self.generate_magic_items, 0, 0),
           ("📜 Spell Scrolls Shop", self.generate_scrolls, 0, 1),
           ("🧪 Potion Shop", self.generate_potions, 0, 2),
           ("⚔️ Basic Items Shop", self.generate_basic, 1, 0),
           ("🎰 Combined Shop", self.generate_combined, 1, 1),
       ]
       ...
       link_note = ttk.Label(
           button_frame,
           text="🔗 Items with URLs will appear in Quick Links for easy access.",
           font=('Segoe UI', 9, 'italic'),
           foreground='#666666'
       )
       link_note.grid(row=4, column=0, columnspan=3)
